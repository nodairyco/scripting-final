...
export default function ShippingInfo() {
  ...
  const [form, setForm] = useState({
    contact: '', name: '', secondName: '', address: '', note: '', city: '', postal: '', province: '', country: '', saveInfo: false
  });
      ...
      <div className={css.container}>
        <div className={css.left}>
          <div className={css.breadcrumbs}>
            <span className={css.active}>Cart</span> <span>›</span> <span className={css.active}>Details</span> <span>›</span> <span>Shipping</span> <span>›</span> <span>Payment</span>
          </div>
          <form className={css.form} onSubmit={handleSubmit}>
            <div className={formCss.formHeader}>Contact</div>
            <input name="contact" type="text" placeholder="Email or mobile phone number" value={form.contact} onChange={handleChange} className={formCss.inputField} />
            ...
            <div className={formCss.inputRow}>
              <div style={{flex: 1}}>
                <input name="name" type="text" placeholder="Name" value={form.name} onChange={handleChange} className={formCss.inputField} />
                {errors.name && <div className={formCss.errorMsg}>{errors.name}</div>}
              </div>
              <div style={{flex: 1}}>
                <input name="secondName" type="text" placeholder="Second Name" value={form.secondName} onChange={handleChange} className={formCss.inputField} />
                {errors.secondName && <div className={formCss.errorMsg}>{errors.secondName}</div>}
              </div>
            </div>
            <input name="address" type="text" placeholder="Address and number" value={form.address} onChange={handleChange} className={formCss.inputField} />
            {errors.address && <div className={formCss.errorMsg}>{errors.address}</div>}
            <input name="note" type="text" placeholder="Shipping note (optional)" value={form.note} onChange={handleChange} className={formCss.inputField} />
            <div className={formCss.inputRow}>
              <div style={{flex: 1}}>
                <input name="city" type="text" placeholder="City" value={form.city} onChange={handleChange} className={formCss.inputField} />
                {errors.city && <div className={formCss.errorMsg}>{errors.city}</div>}
              </div>
              <div style={{flex: 1}}>
                <input name="postal" type="text" placeholder="Postal Code" value={form.postal} onChange={handleChange} className={formCss.inputField} />
              ...
              <div style={{flex: 1}}>
                ...
                >
                  ...
                  {regionList.map(region => (
                    <option key={region.shortCode || region.name} value={region.name}>{region.name}</option>
                  ))}
                </select>
            ...
            <div className={formCss.buttonRow}>
              <button type="button" className={css.link} onClick={() => navigate('/cart')}>Back to cart</button>
          ...
          {cartItems.length > 0 && (
            <div className={css.summary}>
              ...
              <div className={css.totals}>
                ...
                <div className={css.line}><span>Shipping</span><span>Calculated at the next step</span></div>
                <div className={css.line}><span>Total</span><span className={css.total}>$ {subtotal.toFixed(2)}</span></div>
              </div>
            </div>
          )}
        </div>
